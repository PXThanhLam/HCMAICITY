...
def plot_tracking(image, tlwhs, obj_ids, scores=None, frame_id=0, fps=0., ids2=None,out_track=None):
    im = np.ascontiguousarray(np.copy(image))
    im_h, im_w = im.shape[:2]

    top_view = np.zeros([im_w, im_w, 3], dtype=np.uint8) + 255

    text_scale = max(1, image.shape[1] / 1000.)
    ...
    for idx,track in enumerate(out_track):
        ...
        cv2.putText(im,str(frame_id)+' '+ str(id)+' '+str(classes)+' '+str(movement), (im_w-counting_width, (idx+1)*space), cv2.FONT_HERSHEY_PLAIN, text_scale, (0, 255, 0),
                    thickness=text_thickness)
...
def plot_detections(image, tlbrs, scores=None, color=(255, 0, 0), ids=None,box_occlusion=None,types=None):
    im = np.copy(image)
    ...
    for i, det in enumerate(tlbrs):
        ...
        if scores is not None:
            ...
            cv2.putText(im, text, (x1, y1 + 30), cv2.FONT_HERSHEY_PLAIN, text_scale, (0, 255, 255),
                        thickness=thickness)
        if box_occlusion is not None:
            text = box_occlusion[i]
            cv2.putText(im, text, (x1, y1 + 80), cv2.FONT_HERSHEY_PLAIN,1, (0, 255, 255),
                        thickness=thickness)
