...
class STrack(BaseTrack):
    ...
    def __init__(self, tlwh, score, vehicle_type, buffer_size=30,temp_feat=None):
...
class JDETracker(object):
    def __init__(self, opt,polygon, paths, polygon2=None,frame_rate=30):
        ...
        self.line2=[polygon[1],polygon[2]] if len(self.polygon)==5 else [self.polygon[0],self.polygon[1]] if len(self.polygon)==4 else None
        self.line1=[polygon[4],polygon[3]] if len(self.polygon)==5 else [self.polygon[3],self.polygon[2]] if len(self.polygon)==4 else None
        self.two_polygon_system=True
        self.warmup_frame=6
        #Hacky for video 5
        self.polygon[3]=[910,520]
        self.polygon[4]=[0,520]
        self.polygon[1]=[290,280]
        self.polygon[2]=[720,280]
        self.polygon[0]=[0,440]
    ...
    def update(self, im_blob, img0):
        ...
        if self.opt.detection_model=='Efficient':
            with torch.no_grad():
                ...
                for j in range(len(out[0]['rois'])):
                    ...
                    if obj in self.obj_interest:
                        ...
                        #bike,bicycle
                        if (y1+y2)/2>0.5*height and float(out[0]['scores'][j])<=0.1:
                            continue
                        
                        if obj not in self.person_or_motorcycle and float(out[0]['scores'][j])>=0.1:
            ...
            for j in range(len(labels)):
                ...
                if obj in self.obj_interest:
                    x1, y1, x2, y2 = boxes[j]
                    #bike,bicycle
                    # if (y1+y2)/2>0.5*height and float(scores[j])<=0.1:
                    #     continue
                   
                    if obj not in self.person_or_motorcycle and float(scores[j])>=self.opt.det_thres:
                        bbox.append([x1, y1, x2, y2])
        ...
        if len(bbox) > 0:
            '''Detections'''
            detections = [STrack(STrack.tlbr_to_tlwh(tlbr), sco, clas, 30) for
                          (tlbr, sco,clas) in zip(bbox,score,types)]
        ...
        for inew in u_detection:
            ...
            if track.score < self.det_thresh or track.occlusion_status==True or  check_bbox_outside_polygon(init_polygon,track.tlbr):
                continue
            # track_types=self.person_or_motorcycle[0] if tlbrs_to_mean_area(track.track_trajectory) <=1500 else track.infer_type()
            if self.frame_id>=5  and not check_bbox_inside_polygon(init_polygon,track.tlbr):#and track_types in self.person_or_motorcycle #person, motorcycle
                continue
        ...
        for idx,current_tracked_tracks in enumerate([refind_stracks,activated_starcks]) :#
            ...
            for track in current_tracked_tracks:
                if tlbrs_to_mean_area(track.track_trajectory) <=4500 :
                ...
                if check_bbox_outside_polygon(out_polygon,track.tlbr) :
                    track.mark_removed()
                    removed_stracks.append(track)
                    if ((len(track.track_frames)>=2 and self.frame_id <=5) or (len(track.track_frames)>=5 and self.frame_id>=self.warmup_frame+6)) and idx==1:########## 4 is confident number of frame
        ...
        for track in lost_stracks:
            print(tlbrs_to_mean_area(track.track_trajectory))
            if tlbrs_to_mean_area(track.track_trajectory) <=4500 :
                track_type= self.person_or_motorcycle[0] #person
            else:
                track_type=track.infer_type()
            ...
            if check_bbox_intersect_or_outside_polygon(out_polygon,track.tlbr) :
                track.mark_removed()
                removed_stracks.append(track)
                if ((len(track.track_frames)>=2 and self.frame_id <=5) or (len(track.track_frames)>=4 and self.frame_id>=self.warmup_frame+6)):
                    track_center=[ [(x[0]+x[2])/2,(x[1]+x[3])/2] for x in track.track_trajectory]
                    movement_id=counting_moi(self.paths,[(track_center[0],track_center[-1])])[0]
                    line_interest=self.line1 if str(movement_id)=='1' else self.line2
                    out_direction='bottom' if str(movement_id)=='1' else 'up'
        ...
        # person_mov_2_tracklet=[]
        # out_of_polygon_tracklet_copy=[]
        # for ans in out_of_polygon_tracklet:
        #     if ans[2] in self.person_or_motorcycle and str(ans[3])==2:
        #         person_mov_2_tracklet.append(ans)
        #     else:
        #         out_of_polygon_tracklet_copy.append(ans)
        # for idx in range((len(person_mov_2_tracklet)+1)//2):
        #     out_of_polygon_tracklet_copy.append(person_mov_2_tracklet[idx])
        # out_of_polygon_tracklet=out_of_polygon_tracklet_copy
...
def kalman_predict_out_line(track,line,out_direction):
    ...
    predict_thres=20 if out_direction=='up' else 0
    max_long_predict=40 if out_direction=='up' else 8# if track.infer_type() in ['person','motorcycle','biycycle'] else 10
...
def heuristic_occlusion_detection(detections,thres=0.5): #0.6
    ...
    for idx,detection_tlbrscore in enumerate(detection_tlbrscores):
        ...
        num_invalid_thres2=len(np.where(box_ious>0.55)[0]) #0.75

        detections[idx].iou_box=np.sort(box_ious)[-2] if len(box_ious)>1 else None
        occ_iou.append(detections[idx].iou_box)
        if num_invalid >=2 or (num_invalid_thres2>=2 and box_area>10000):
            occ_status.append(True)
            detections[idx].occlusion_status=True
        else:
            occ_status.append(False)
