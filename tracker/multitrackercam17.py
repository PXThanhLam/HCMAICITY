...
class JDETracker(object):
    def __init__(self, opt,polygon, paths, polygon2=None,frame_rate=30):
        ...
        self.line1=[self.polygon[0],self.polygon[-1]]
        self.line3=[self.polygon[0],self.polygon[1]] 
        ...
        with torch.no_grad():
            ...
            for j in range(len(out[0]['rois'])):
                ...
                if obj in self.obj_interest:
                    ...
                    #bike,bicycle
                    # if (y1+y2)/2>0.36*height and float(out[0]['scores'][j])<=0.15:
                    #     continue
                    # elif (y1+y2)/2>0.4*height and float(out[0]['scores'][j])<=0.25 and obj not in self.person_or_motorcycle:
                    #     continue
                    if obj not in self.person_or_motorcycle and float(out[0]['scores'][j])>=0.25:
                        bbox.append([x1, y1, x2, y2])
                        score.append( float(out[0]['scores'][j]))
                        types.append(obj)
                        if (y1+y2)/2<0.3*height and  ((y2-y1)>=110 or (x2-x1)>=110):
                            huge_vehicles.append( True)
                            continue
                        if (y2-y1)<=huge_box_thres and (x2-x1)<=huge_box_thres:
                    ...
                    if ((len(track.track_frames)>=4 and self.frame_id <=5) or (len(track.track_frames)>=5 and self.frame_id>=self.warmup_frame+5)) and idx==1:########## 4 is confident number of frame
                        ...
                        if track_center[-1][1]<=220:
                            if track_center[-1][0]>=170:
                                ...
                                continue
                        elif ((track_center[-1][0]>=950 and track_type in self.person_or_motorcycle) or (track.track_trajectory[0][2]>=800)) and track_center[-1][1]>=400 :
                            movement_id='3'
                            ...
                            continue
                        if movement_id!='2':
                            if tlbrs_to_mean_area(track.track_trajectory) <=8000 :
                                track_type= self.person_or_motorcycle[0] 
                        ...
                        #     continue
                        if box_line_relative(track.track_trajectory[-2],self.line3)=='cross' or box_line_relative(track.track_trajectory[-1],self.line3)=='cross':
                            continue
...
def kalman_predict_out_line(track,line,out_direction):
    ...
    max_long_predict=7 if out_direction=='up' else 3
