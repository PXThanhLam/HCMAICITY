...
class STrack(BaseTrack):
    ...
    def infer_type(self):
        ...
        if types=='car' and len(np.where(np.asarray(self.vehicle_types_list)=='motorcycle')[0])/len(self.vehicle_types_list)>=0.3:
            return 'motorcycle'
        return types
...
class JDETracker(object):
    def __init__(self, opt,polygon, paths, polygon2=None,frame_rate=30):
        ...
        self.line1=[self.polygon[3],self.polygon[0]]
        self.line2=[self.polygon[0],self.polygon[1]]
        self.line3=[self.polygon[2],self.polygon[3]]
    ...
    def update(self, im_blob, img0):
        ...
        with torch.no_grad():
            ...
            for j in range(len(out[0]['rois'])):
                ...
                if obj in self.obj_interest:
                    ...
                    #     continue
                    if obj not in self.person_or_motorcycle and float(out[0]['scores'][j])>=0.18:
            ...
            for track in current_tracked_tracks:
                ...
                if check_bbox_outside_polygon(out_polygon,track.tlbr) :
                    ...
                    if ((len(track.track_frames)>=2 and self.frame_id <=5) or (len(track.track_frames)>=5 and self.frame_id>=self.warmup_frame+5)) and idx==1:########## 4 is confident number of frame
                        ...
                        movement_id=self.heusristic_mov_detection(track,track_center)#counting_moi(self.paths,[(track_center[0],track_center[-1])])[0]                        
                        frame_id=self.frame_id#+kalman_predict_out_line(track,line_interest,out_direction)
                        orig_type=copy.copy(track_type)
                        track_type=self.heuristictype_detection(track.track_trajectory,movement_id,orig_type)
                        out_of_polygon_tracklet.append((frame_id,track.track_id,track_type,movement_id))
                        print(str(track.track_id)+' : '+str(tlbrs_to_mean_area(track.track_trajectory)))
                else:
        ...
        for track in lost_stracks:
            ...
            if check_bbox_intersect_or_outside_polygon(out_polygon,track.tlbr) :
                track.mark_removed()
                removed_stracks.append(track)
                if ((len(track.track_frames)>=2 and self.frame_id <=5) or (len(track.track_frames)>=5 and self.frame_id>=self.warmup_frame+5)):
                    track_center=[ [(x[0]+x[2])/2,(x[1]+x[3])/2] for x in track.track_trajectory]
                    movement_id=self.heusristic_mov_detection(track,track_center)                    
                    frame_id=self.frame_id+5#+kalman_predict_out_line(track,line_interest,out_direction)
                    orig_type=copy.copy(track_type)
                    track_type=self.heuristictype_detection(track.track_trajectory,movement_id,orig_type)
                    out_of_polygon_tracklet.append((frame_id,track.track_id,track_type,movement_id))
                    print(str(track.track_id)+' : '+str(tlbrs_to_mean_area(track.track_trajectory)))
