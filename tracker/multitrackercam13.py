...
class JDETracker(object):
    def __init__(self, opt,polygon, paths, polygon2=None,frame_rate=30):
        ...
        self.warmup_frame=9 if self.two_polygon_system else 0
    ...
    def update(self, im_blob, img0):
        ...
        for inew in u_detection:
            ...
            if not track.huge_vehicle:
                if track.tlwh[2]*track.tlwh[3]<8000:
                    track_init_polygon=init_polygon 
                else:
                    track_init_polygon=four_wheel_polygon
            else:
        ...
        for idx,current_tracked_tracks in enumerate([refind_stracks,activated_starcks]) :#
            ...
            for track in current_tracked_tracks:
                ...
                if (tlbrs_to_mean_area(track.track_trajectory) <=4000 and str(movement_id)=='1') or (tlbrs_to_mean_area(track.track_trajectory) <=1500 and str(movement_id)=='2'):
                ...
                if check_bbox_outside_polygon(out_polygon,track.tlbr) :
                    ...
                    removed_stracks.append(track)
                    #embarassing hacking :))
                    if self.frame_id<=10 and movement_id=='1':
                        continue
                    if ((len(track.track_frames)>=3 and self.frame_id <=5) or (len(track.track_frames)>=5 and self.frame_id>=self.warmup_frame)) and idx==1:########## 4 is confident number of frame
        ...
        for track in lost_stracks:
            ...
            if check_bbox_intersect_or_outside_polygon(out_polygon,track.tlbr) :
                track.mark_removed()
                removed_stracks.append(track)
                if ((len(track.track_frames)>=3 and self.frame_id <=5) or (len(track.track_frames)>=5 and self.frame_id>=self.warmup_frame)):
                    line_interest=self.line1 if str(movement_id)=='1' else self.line2
                    out_direction='bottom' if str(movement_id)=='1' else 'up'
...
def heuristic_occlusion_detection(detections,thres=0.6): #0.5
    ...
    for idx,detection_tlbrscore in enumerate(detection_tlbrscores):
        ...
        num_invalid_thres2=len(np.where(box_ious>0.9)[0]) #0.55
        ...
        if num_invalid >=2 :
            ...
            new_detection_pool.append(detections[idx])
        if (num_invalid_thres2>=2 and box_area>10000):
