...
class JDETracker(object):
    ...
    def update(self, im_blob, img0):
        ...
        dists = matching.gate_cost_matrix6(self.kalman_filter, dists, strack_pool, detections,True)
        ...
        for inew in u_detection:
            ...
            if track.score < self.det_thresh or track.occlusion_status==True or  check_bbox_outside_polygon(self.polygon,track.tlbr):
                continue
            if self.frame_id>=5  and not check_bbox_inside_polygon(self.polygon,track.tlbr):  #['bicycle',  'motorcycle']
                continue
        ...
        if self.frame_id>=5:
            for track in lost_stracks:
                if check_bbox_intersect_or_outside_polygon(self.polygon,track.tlbr) and track.infer_type() in self.person_or_motorcycle : #['bicycle',  'motorcycle']
                    track.mark_removed()
                    removed_stracks.append(track)
                    track_type=track.infer_type()
                    track_center=[ [(x[0]+x[2])/2,(x[1]+x[3])/2] for x in track.track_trajectory]
