...
class STrack(BaseTrack):
    ...
    def infer_type(self):
        ...
        if types=='car' and len(np.where(np.asarray(self.vehicle_types_list)=='person')[0])/len(self.vehicle_types_list)>=0.25:
            return 'person'
        elif self.vehicle_types_list[-1]=='person':
            return 'person'
        if self.track_id==33:
            print(self.vehicle_types_list)
        return types
...
class JDETracker(object):
    def __init__(self, opt,polygon, paths, polygon2=None,frame_rate=30):
        ...
        self.line1=[self.polygon[0],self.polygon[3]]
        self.line2=[self.polygon[0],self.polygon[1]]
        self.line3=[self.polygon[1],self.polygon[2]]
    ...
    def box_cross_line(self,bbox_list,line):
        for bbox in bbox_list:
            if box_line_relative(bbox,line)=='cross' :
                return True
    def heusristic_mov_detection(self,track,track_center):
        
        if self.box_cross_line(track.track_trajectory[-4:],self.line1)  :
               
            paths={'1':self.paths['1'],'5':self.paths['5']}
            mov_id= counting_moi(paths,[(track_center[0],track_center[-1])])
            return mov_id[0]
        elif self.box_cross_line(track.track_trajectory[-4:] , self.line3) :
            paths={'2':self.paths['2'],'4':self.paths['4']}
            mov_id= counting_moi(paths,[(track_center[0],track_center[-1])])
            return mov_id[0]
        elif self.box_cross_line(track.track_trajectory[-4:],self.line2):
            paths={'3':self.paths['3'],'8':self.paths['6']}
            mov_id= counting_moi(paths,[(track_center[0],track_center[-1])])
            return mov_id[0]
        ...
        with torch.no_grad():
            ...
            for j in range(len(out[0]['rois'])):
                ...
                if obj in self.obj_interest:
                    ...
                    #bike,bicycle
                    if (y1+y2)/2>0.5*height and float(out[0]['scores'][j])<=0.15:
                        continue
                    
                    if obj not in self.person_or_motorcycle and float(out[0]['scores'][j])>=0.15:
