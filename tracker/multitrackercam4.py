...
class STrack(BaseTrack):
    ...
    def infer_type(self):
        ...
        if detection_model=='Efficient':
            ...
            types=most_frequent(self.vehicle_types_list)
            if types=='car' and len(np.where(np.asarray(self.vehicle_types_list)=='truck')[0])/len(self.vehicle_types_list)>=0.3:
                return 'truck'
            if types=='car' and  len(np.where(np.asarray(self.vehicle_types_list)=='bus')[0])/len(self.vehicle_types_list)>=0.3:
                return 'bus'
            return types
    @property
...
class JDETracker(object):
    def __init__(self, opt,polygon, paths, polygon2=None,frame_rate=30):
        ...
        self.line2=[self.polygon[1],self.polygon[2]] if len(self.polygon)==5 else [self.polygon[0],self.polygon[1]] if len(self.polygon)==4 else None
        self.line1=[self.polygon[4],self.polygon[3]] if len(self.polygon)==5 else [self.polygon[3],self.polygon[2]] if len(self.polygon)==4 else None
        self.two_polygon_system=True
        self.warmup_frame=6
        #Hacky for video 4
        self.polygon[3]=[910,610]
        self.polygon[4]=[0,610]
    ...
    def update(self, im_blob, img0):
            ...
            with torch.no_grad():
                ...
                for j in range(len(out[0]['rois'])):
                    ...
                    if obj in self.obj_interest:
                        ...
                        #bike,bicycle
                        if (y1+y2)/2>0.5*height and float(out[0]['scores'][j])<=0.35:
                            continue
        ...
        for track in lost_stracks:
            ...
            if check_bbox_intersect_or_outside_polygon(out_polygon,track.tlbr) :
                track.mark_removed()
                removed_stracks.append(track)
                if ((len(track.track_frames)>=2 and self.frame_id <=5) or (len(track.track_frames)>=6 and self.frame_id>=self.warmup_frame+6)):
                    track_center=[ [(x[0]+x[2])/2,(x[1]+x[3])/2] for x in track.track_trajectory]
                    movement_id=counting_moi(self.paths,[(track_center[0],track_center[-1])])[0]
                    line_interest=self.line1 if str(movement_id)=='1' else self.line2
                    out_direction='bottom' if str(movement_id)=='1' else 'up'
...
def kalman_predict_out_line(track,line,out_direction):
    ...
    max_long_predict=30 if out_direction=='up' else 2 if track.infer_type() in ['person','motorcycle','biycycle'] else 10
