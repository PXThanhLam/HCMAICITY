    ...
    def infer_type(self):
        ...
        if opt.detection_model=='Efficient':
            ...
            if types=='bus' and len(np.where(np.asarray(self.vehicle_types_list)=='truck')[0])/len(self.vehicle_types_list)>=0.3:
                return 'truck'
            if len(np.where(np.asarray(self.vehicle_types_list)=='tricycle')[0])/len(self.vehicle_types_list)>=0.3:
                return 'tricycle'
            return types
...
class JDETracker(object):
    ...
    def update(self, im_blob, img0):
        ...
        if self.opt.detection_model=='Efficient':
            with torch.no_grad():
                ...
                for j in range(len(out[0]['rois'])):
                    ...
                    if obj in self.obj_interest:
                        ...
                        #bike,bicycle
                        if (y1+y2)/2>0.5*height and float(out[0]['scores'][j])<=self.opt.near_cam_thres:
                            continue
                        elif (y1+y2)/2>0.4*height and float(out[0]['scores'][j])<=self.opt.near_cam_big_veh_thres and obj not in self.person_or_motorcycle:
                            continue
                        if obj not in self.person_or_motorcycle and float(out[0]['scores'][j])>=self.opt.big_veh_thres:
            ...
            for j in range(len(labels)):
                ...
                if obj in self.obj_interest:
                    x1, y1, x2, y2 = boxes[j]
                    #bike,bicycle
                    if (y1+y2)/2>0.5*height and float(scores[j])<=self.opt.near_cam_thres:
                        continue
                    elif (y1+y2)/2>0.4*height and float(scores[j])<=self.opt.near_cam_big_veh_thres and obj not in self.person_or_motorcycle:
                        continue
                    if obj not in self.person_or_motorcycle and float(scores[j])>=self.opt.big_veh_thres:
                        bbox.append([x1, y1, x2, y2])
        ...
        match_thres=150 if self.opt.detection_model=='Efficient' else 100
        dists=np.zeros(shape=(len(strack_pool),len(detections)))
        if self.opt.detection_model=='FasterRcnn':
            dists = matching.gate_cost_matrix4(self.kalman_filter, dists, strack_pool, detections,type_diff=True) 
        else:
        ...
        for idx,current_tracked_tracks in enumerate([refind_stracks,activated_starcks]) :#
            ...
            for track in current_tracked_tracks:
                ...
                if track_type in self.person_or_motorcycle:
                    ...
                    p_type='four_wheel'
                if track_type=='truck' and tlbrs_to_mean_area(track.track_trajectory)<=6000 and track.track_trajectory[-1][0]>=850:
                    track_type='undetermine'
        ...
        for track in lost_stracks:
            ...
            if check_bbox_intersect_or_outside_polygon(out_polygon,track.tlbr) :
                ...
                if len(track.track_frames)>=8 and track.track_trajectory[-1][3]>=350 and movement_id=='1':
...
def kalman_predict_out_line(track,line,out_direction):
    ...
    max_long_predict=5 if out_direction=='up' else 2 if track.infer_type() in ['person','motorcycle','biycycle','motor','car'] else 10
...
def heuristic_occlusion_detection(detections,thres=0.6): #0.5
    ...
    for idx,detection_tlbrscore in enumerate(detection_tlbrscores):
        ...
        high_inter_bboxes1=np.where(np.logical_and(box_ious>thres, delta_scores >=0))[0]
        num_invalid= len(high_inter_bboxes1) #-0.03
        high_inter_bboxes2=np.where(box_ious>0.85)[0]
        num_invalid_thres2=len(high_inter_bboxes2) #0.55
        ...
        if num_invalid >=2 :
            ...
            if box_area<=3000 and detection_tlbrscore[4] >=0.75:
                new_detection_pool.append(detections[idx])
        else:
            new_detection_pool.append(detections[idx])
        # overlap_det_classes=[]
        # for idx in high_inter_bboxes2:
        #     overlap_det_classes.append(detections[idx].vehicle_types_list[-1])
        # if (num_invalid_thres2>=2 and box_area>15000 and 'truck' in overlap_det_classes ):
