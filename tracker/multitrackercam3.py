...
class STrack(BaseTrack):
    ...
    def infer_type(self):
        ...
        if types=='bus' and len(np.where(np.asarray(self.vehicle_types_list)=='truck')[0])/len(self.vehicle_types_list)>=0.3:
            return 'truck'
        if types=='car' and len(np.where(np.asarray(self.vehicle_types_list)=='person')[0])/len(self.vehicle_types_list)>=0.3:
            return 'person'
        x1,y1,x2,y2=self.track_trajectory[-1]
        s=(x2-x1)*(y2-y1)
        if types=='car' and len(np.where(np.asarray(self.vehicle_types_list)=='truck')[0])/len(self.vehicle_types_list)>=0.3 and s>=55000:
            return 'truck'
    ...
    def update(self, im_blob, img0):
        ...
        with torch.no_grad():
            ...
            for j in range(len(out[0]['rois'])):
                ...
                if obj in self.obj_interest:
                    ...
                    #bike,bicycle
                    if (y1+y2)/2>325 and float(out[0]['scores'][j])<=0.2:
                        continue
                    if (y1+y2)/2<0.5*height and float(out[0]['scores'][j])<=self.opt.near_cam_thres:
                        continue
                    if (y1+y2)/2>0.5*height and float(out[0]['scores'][j])<=self.opt.near_cam_thres:
                        continue
                    elif (y1+y2)/2>0.35*height and float(out[0]['scores'][j])<=self.opt.near_cam_big_veh_thres and obj not in self.person_or_motorcycle:
                        continue
        ...
        for idx,current_tracked_tracks in enumerate([refind_stracks,activated_starcks]) :#
            ...
            for track in current_tracked_tracks:
                ...
                if (tlbrs_to_mean_area(track.track_trajectory) <=6500 and str(movement_id)=='1') or(tlbrs_to_mean_area(track.track_trajectory) <=2000 and str(movement_id)=='2') :
        ...
        for track in lost_stracks:
            track_center=[ [(x[0]+x[2])/2,(x[1]+x[3])/2] for x in track.track_trajectory]
            movement_id=counting_moi(self.paths,[(track_center[0],track_center[-1])])[0]
            if (tlbrs_to_mean_area(track.track_trajectory) <=6500 and str(movement_id)=='1') or(tlbrs_to_mean_area(track.track_trajectory) <=3000 and str(movement_id)=='2') :
            ...
            if track.track_id==3:
                print(track.vehicle_types_list)
            if check_bbox_intersect_or_outside_polygon(out_polygon,track.tlbr) :
                ...
                if len(track.track_frames)>=7 and track.track_trajectory[-1][3]>=350 and movement_id=='1':
                    out_of_polygon_tracklet.append((self.frame_id+6,track.track_id,track_type,movement_id))
                    continue
...
def kalman_predict_out_line(track,line,out_direction):
    ...
    max_long_predict=35 if out_direction=='up' else 2 if track.infer_type() in ['person','motorcycle','biycycle'] else 5
