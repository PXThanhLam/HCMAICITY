...
class STrack(BaseTrack):
    ...
    def infer_type(self):
        ...
        if types=='bus' and len(np.where(np.asarray(self.vehicle_types_list)=='truck')[0])/len(self.vehicle_types_list)>=0.21:
            return 'truck'
        return types
        ...
        with torch.no_grad():
            ...
            for j in range(len(out[0]['rois'])):
                ...
                if obj in self.obj_interest:
                    ...
                    #bike,bicycle
                    if (y1+y2)/2>0.3*height and float(out[0]['scores'][j])<=0.35:
                        continue
                    elif (y1+y2)/2>0.4*height and float(out[0]['scores'][j])<=0.4 and obj not in self.person_or_motorcycle:
                            continue
                    if obj not in self.person_or_motorcycle and float(out[0]['scores'][j])>=0.35:
...
def kalman_predict_out_line(track,line,out_direction):
    ...
    max_long_predict=40 if out_direction=='up' else 2 if track.infer_type() in ['person','motorcycle','biycycle'] else 10
